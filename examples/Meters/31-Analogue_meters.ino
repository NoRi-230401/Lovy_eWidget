...
MeterWidget amps = MeterWidget(&sprite);
MeterWidget volts = MeterWidget(&sprite);
MeterWidget ohms = MeterWidget(&sprite);
...
void setup(void)
{
  ...
  void *p = sprite.createSprite(lcd.width(), lcd.height());
  if ( p == NULL ) {
    delay(5000);
