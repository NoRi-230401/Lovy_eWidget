...
ButtonWidget btnL = ButtonWidget(&sprite);
ButtonWidget btnR = ButtonWidget(&sprite);
...
void initButtons()
{
  uint16_t x = (lcd.width() - BUTTON_W) / 2;
  uint16_t y = lcd.height() / 2 - BUTTON_H - 10;
...
void loop()
{
  ...
  if (millis() - scanTime >= 50)
  {
    ...
    bool pressed = lcd.getTouch(&t_x, &t_y);
    scanTime = millis();
