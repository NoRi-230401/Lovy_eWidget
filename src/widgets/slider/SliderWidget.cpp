...
***************************************************************************************/
SliderWidget::SliderWidget(LGFX_Sprite *tft)
{
  _tft = tft;
}
...
void SliderWidget::drawSlider(uint16_t x, uint16_t y)
{
  if(_horiz) {
    ...
    _tft->fillSmoothRoundRect(_xpos, _ypos - _slotWidth/2, _slotLength, _slotWidth, _slotWidth/2, _slotColor);
    ...
    _xpos = x + _kwidth/2 + 2;
    _ypos = y + 2;
    _tft->fillSmoothRoundRect(_xpos - _slotWidth/2, _ypos, _slotWidth, _slotLength, _slotWidth/2, _slotColor);
...
void SliderWidget::moveTo(int16_t val)
{
  ...
  LGFX_Sprite knob_spr(_tft);
  uint16_t kd = (_horiz ? _kwidth : _kheight);
  uint16_t kpos = map(val, _sliderMin, _sliderMax, _slotWidth/2 + 1, _slotLength - _slotWidth/2 - kd - 1);

  knob_spr.createSprite(_kwidth + 2, _kheight + 2);

  if (_usdelay == 0 && abs(kpos - _kposPrev) > kd + 1) {
    drawKnob(kpos, &knob_spr);
    _kposPrev = kpos;
  }
  else {
    int8_t dp = 1;
    if (kpos < _kposPrev) dp = -1;
    while (kpos != _kposPrev) {_kposPrev += dp;  drawKnob(_kposPrev, &knob_spr); delayMicroseconds(_usdelay);}
  }

  knob_spr.deleteSprite();
}
...
***************************************************************************************/
void SliderWidget::drawKnob(uint16_t kpos, LGFX_Sprite* spr)
{
  uint16_t x, y;
  ...
  if(_horiz) {
    ...
    spr->fillRect(0, _kheight/2 - _slotWidth/2 + 1, _kwidth + 2, _slotWidth, _slotColor);
  }
  else  {
    x = _xpos - _kwidth/2 - 1;
    y = _ypos + kpos - 1;
    spr->fillRect(_kwidth/2 - _slotWidth/2 + 1, 0, _slotWidth, _kheight + 2, _slotColor);
  }

  if (_usdelay == 0 && abs(kpos - _kposPrev) > (_horiz ? _kwidth : _kheight) + 1) {
    if(_horiz) spr->pushSprite(_xpos + _kposPrev, y);
    else spr->pushSprite(_xpos, y + _kposPrev);
  }

  // Draw slider outline
  spr->fillSmoothRoundRect(1, 1, _kwidth, _kheight, _kradius, _kcolor1);

  // Draw marker stripe
  if(_kcolor1 != _kcolor2) {
    if(_horiz) {
      spr->drawFastVLine(_kwidth/2 + 1, 1, _kheight, _kcolor2);
    }
    else  {
      spr->drawFastHLine(1, _kheight/2 + 1, _kwidth, _kcolor2);
    }
  }
  spr->pushSprite(x,y);
}
