...
void MeterWidget::analogMeter(uint16_t x, uint16_t y, float startScale, float endScale, const char *units, const char *s0, const char *s1, const char *s2, const char *s3, const char *s4)
{
  ...
  for (int i = -50; i < 51; i += 5)
  {
    ...
    if (i % 25 == 0)
    {
      ...
      case -2:
        ntft->drawCentreString(ms0, x0, y0 - 12, &Font2);
        break;
      case -1:
        ntft->drawCentreString(ms1, x0, y0 - 9, &Font2);
        break;
      case 0:
        ntft->drawCentreString(ms2, x0, y0 - 6, &Font2);
        break;
      case 1:
        ntft->drawCentreString(ms3, x0, y0 - 9, &Font2);
        break;
      case 2:
        ntft->drawCentreString(ms4, x0, y0 - 12, &Font2);
        break;
      }
    }
